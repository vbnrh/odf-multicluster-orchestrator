		...
		if eachSecret.Name == req.Name {
			...
			if secretType := eachSecret.Labels[utils.SecretLabelTypeKey]; secretType == string(utils.SourceLabel) {
				...
				if sourceSecretPointer != nil {
					...
					logger.Error(err, "Cannot have more than one source secrets with the same name", "request", req, "source-secret", sourceSecretPointer, "namespace", sourceSecretPointer.Namespace)
					return err
				}
